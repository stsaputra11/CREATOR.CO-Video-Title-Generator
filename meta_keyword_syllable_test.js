...
function stripRepeatedConcepts(s){
...
function estimatePhraseSyllables(value){
 return metaPhrase(value)
   .split(" ")
   .filter(Boolean)
   .reduce((sum,word)=>sum+estimateWordSyllables(word),0);
}
...
function metaKeywordScore(value,mainKeyword,niche,useCase){
 ...
 // Strong priority for concise meta tags: ideally no more than 4 syllables.
 if(syllables<=2) score+=32;
 else if(syllables<=4) score+=40;
 else if(syllables<=6) score+=8;
 else score-=30;
...
function buildMetaTagKeywords(mainKeyword,relatedKeywords,niche,useCase){
 ...
 const others=[...best.values()]
   ...
   .sort((a,b)=>{
     const aShort=estimatePhraseSyllables(a.phrase)<=4 ? 1 : 0;
     const bShort=estimatePhraseSyllables(b.phrase)<=4 ? 1 : 0;
...
for(const t of tests){
 const out=buildMetaTagKeywords(t.main,t.rel,t.niche,t.use);
 console.log("MAIN:",t.main);
 for(const kw of out){
   console.log(kw,"=>",estimatePhraseSyllables(kw),"syllables");
 }
}
