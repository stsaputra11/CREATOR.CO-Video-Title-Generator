...
let state={keywords:[],moods:[],atmos:[],emotions:[],titles:[]};
...
function generateKeywords(v){
 ...
 for(const c of pool) addCandidate(out,c,v.niche,joined);
 ...
 for(const c of safeFallback){
   if(out.length>=9) break;
   addCandidate(out,c,v.niche,joined);
 }

 state.keywords=out.slice(0,9);
}
...
function generateAtmos(v){
 ...
 function phrasesForOption(opt){
   ...
   (map[opt] || [`${opt} ambience`, `quiet ${opt}`, `calm ${opt} atmosphere`]).forEach(push);
 ...
 state.atmos=candidates.slice(0,10);
}
...
async function run(mode="all"){
 try{
   ...
   $("loadingText").textContent=mode==="titles"?"Generating Titles...":"Analyzing SEO Keywords...";
   await new Promise(r=>setTimeout(r,250));

   if(mode==="titles" && (!state.keywords.length || !state.atmos.length || !state.emotions.length)){
     generateKeywords(v); generateMoods(v); generateEmotions(v); generateAtmos(v);
   }
   if(mode==="keywords"){ generateKeywords(v); generateMoods(v); generateEmotions(v); if(!state.atmos.length)generateAtmos(v); }
   if(mode==="atmos"){ generateAtmos(v); if(!state.keywords.length)generateKeywords(v); if(!state.moods.length)generateMoods(v); if(!state.emotions.length)generateEmotions(v); }
   if(mode==="all"){ generateKeywords(v); generateMoods(v); generateEmotions(v); generateAtmos(v); }
...
$("exportSpreadsheet").onclick=exportSpreadsheet;
$("reset").onclick=()=>{
 ...
 $("placeholder").style.display="block";
 $("warning").style.display="none";
 state={keywords:[],moods:[],atmos:[],emotions:[],titles:[]};
};
